...
class ThisDataset(utils.Dataset):
    ...
    def load_this(self, dataset_dir, subset):
        ...
        for a in annotations:
            ...
            self.add_image(
                ...
                polygons=rects, classes=classes)
    ...
    def load_mask(self, image_id):
        ...
        mask = np.zeros([info["height"], info["width"], len(info["polygons"])],
                        dtype=np.uint8)
        classes = np.zeros(len(info["polygons"]), dtype=np.int32)
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            classes[i] = class_map[info['classes'][i]["object_name"]]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Faster-CNN to detect people.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'tag'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/people/dataset/",
                        help='Directory of the People dataset')
